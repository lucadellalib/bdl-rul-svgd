...
"""Train a Bayesian model via Bayes by Backprop (BBB)."""

# To run an experiment, open a terminal and run:
# python train_bbb.py [-h] [-r RESTORE] {d3,c2p2} {fd001,fd002,fd003,fd004}
...
def run_experiment(config: "Dict[str, Any]") -> "None":
    ...
    normal_posterior_softplus_inv_std = config["normal_posterior_softplus_inv_std"]
    num_train_mc_samples = config["num_train_mc_samples"]
    num_test_mc_samples = config["num_test_mc_samples"]
    lr = config["lr"]
    ...
    if num_train_mc_samples < 1 or not float(num_train_mc_samples).is_integer():
        raise ValueError(
            f"`num_train_mc_samples` ({num_train_mc_samples}) must be in the integer interval [1, inf)"
        )
    if num_test_mc_samples < 1 or not float(num_test_mc_samples).is_integer():
        raise ValueError(
            f"`num_test_mc_samples` ({num_test_mc_samples}) must be in the integer interval [1, inf)"
        )
    if lr < 0.0:
    ...
    num_train_mc_samples = int(num_train_mc_samples)
    num_test_mc_samples = int(num_test_mc_samples)
    ...
    model = VariationalPosteriorModel(
        ...
        SoftplusInvScaleNormal,
        {
            ...
            "softplus_inv_scale": torch.full(
                (num_parameters,), normal_posterior_softplus_inv_std, requires_grad=True
            ),
        },
    ).to(device)
    print(model)
    ...
    if not test_only:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for data, target in train_loader:
                ...
                # Compute log likelihoods and Kullback-Leibler divergences
                likelihood_args, kl_divs = model(
                    data, num_mc_samples=num_train_mc_samples
                )
                ...
                # Re-weight Kullback-Leibler divergence as in https://arxiv.org/abs/1505.05424
                loss = criterion(log_likelihoods, kl_divs, 1 / len(train_loader))
    ...
    for test_max_target in test_max_targets:
        ...
        for data, target in test_loader:
            ...
            # computation (unused at test time)
            with torch.no_grad():
                likelihood_args, _ = model(data, num_mc_samples=num_test_mc_samples)
            preds = likelihood_args[..., 0]
    ...
    if checkpoint is not None:
        with checkpoint.as_directory():
            prior_samples = torch.normal(
                ...
                size=(num_test_mc_samples, 1),
            ).numpy()
            posterior_samples = torch.normal(
                model.posterior.mean[0].item(),
                model.posterior.stddev[0].item(),
                size=(num_test_mc_samples, 1),
            ).numpy()
            try:
                if plot:
...
if __name__ == "__main__":
    ...
    if experiment_state_dir is not None:
        ...
        run_name = f"bbb_{model_name}_{subset}_{current_time}"
        tuner = Tuner(
            with_resources(
                run_experiment,
            ...
            param_space={
                ...
                "normal_posterior_softplus_inv_std": 1.0,
                "num_train_mc_samples": 10,
                "num_test_mc_samples": 10,
                "lr": 1e-2,
                "step_size": 40,
                "gamma": 0.1,
                "checkpoint_freq": 10,
